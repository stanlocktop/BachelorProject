...
namespace IdentityServer.Quickstart.Account
{
    ...
    public class ExternalController : Controller
    {
        ...
        public async Task<IActionResult> Callback()
        {
            ...
            var r = IdentityServerConstants.ExternalCookieAuthenticationScheme;
            ...
            if (user == null)
            {
                ...
                return View("~/Views/Account/Register.cshtml",new RegistrationInputModel()
                {
                    Email = user.Email,
                    Username = user.UserName,
                    ReturnUrl = result.Properties.Items["returnUrl"],
                    ReturnedMessage = "Реєстрація пройшла успішно. Id вашого користувача " + user.Id + ". Протягом доби ваш аккаунт буде підтверджено адміністратором.",
                });
            ...
            if(user.EmailConfirmed == false)
            {
                return View("~/Views/Account/Register.cshtml", new RegistrationInputModel()
                {
                    ...
                    Username = user.UserName,
                    ReturnUrl = result.Properties.Items["returnUrl"],
                    ReturnedMessage = "Ваш аккаунт не підтверджено. Протягом доби ваш аккаунт буде підтверджено адміністратором.",
                });
            }
        ...
        private async Task<(ApplicationUser user, string provider, string providerUserId, IEnumerable<Claim> claims)> FindUserFromExternalProvider(AuthenticateResult result)
        {
            ...
            // find external user
            var user =await  _userManager.FindByExternalProvider(provider, providerUserId);
        ...
        {
            var user =await  _userManager.AutoProvisionUser(provider, providerUserId, claims.ToList());
            return user;
        }
